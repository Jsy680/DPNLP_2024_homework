...
if __name__ == '__main__':
    ...
    with open('Entropy.txt', 'w') as f:
        for i in range(len(txt_corpus)):
            ...
            f.write(f'小说总字数: {len_char[i]}\n')
            f.write(f'小说分词总个数: {len_words[i]}\n')
            f.write(f'小说平均词长: {round(len_char[i] / len_words[i], 3)}\n')
            f.write(f'小说基于字的中文平均信息熵: {round(char_entropy[i], 2)}\n')
            f.write(f'小说基于词的中文平均信息熵: {round(words_entropy[i], 2)}\n')
        ...
        f.write(f"基于字的中文平均信息熵：{round(sum(char_entropy), 2)}\n")
        f.write(f"基于词的中文平均信息熵：{round(sum(words_entropy), 2)}\n")
        f.close()
